...
.DESCRIPTION
    ...
    1. Downloads the latest mini_installer.nosync.exe from GitHub
...
$installerPath = "$downloadFolder\mini_installer.nosync.exe"
...
try {
    ...
    $asset = $releaseInfo.assets | Where-Object { $_.name -eq "mini_installer.nosync.exe" }
